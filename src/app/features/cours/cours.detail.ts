...
@Component({
  ...
  template: `
    <app-layout>
      <div class="details-container" *ngIf="!loading && cours">
        <!-- En-tête -->
        <mat-card class="header-card">
          <div class="header-content">
            <div class="icon-wrapper">
              <mat-icon>{{getCoursIcon()}}</mat-icon>
            </div>
            ...
            <mat-card-content>
              ...
              <div class="stat-info">
                ...
                <small *ngIf="cours.capacite_max">
                  {{getCapacityPercentage()}}% de capacité
                </small>
              </div>
            </mat-card-content>
          </mat-card>
          ...
          <mat-card class="stat-card ratio">
            <mat-card-content>
              ...
              <div class="stat-info">
                ...
                <p>Ratio T/E</p>
                <mat-chip-set>
                  <mat-chip [color]="getRatio() > 20 ? 'warn' : 'primary'">
                    {{getRatio() > 20 ? 'Élevé' : 'Optimal'}}
                  </mat-chip>
                </mat-chip-set>
              </div>
            </mat-card-content>
...
export class CoursDetailsComponent implements OnInit {
  ...
  getCoursIcon(): string {
    if (this.cours?.code.startsWith('COR')) return 'menu_book';
    if (this.cours?.code.startsWith('HAD')) return 'history_edu';
    if (this.cours?.code.startsWith('FIQ')) return 'gavel';
    if (this.cours?.code.startsWith('TAF')) return 'auto_stories';
    if (this.cours?.code.startsWith('ARA')) return 'translate';
    return 'book';
  }
  ...
  getRatio(): number {
    return this.enseignants.length > 0 
      ? Math.round(this.talibes.length / this.enseignants.length)
      : 0;
  }

  getCapacityPercentage(): number {
    if (!this.cours?.capacite_max) return 0;
    return Math.round((this.talibes.length / this.cours.capacite_max) * 100);
  }
  ...
  manageTalibes(): void {
    this.router.navigate(['/cours', this.cours?.id, 'talibes']);
  }
  ...
  deleteCours(): void {
    if (confirm(`Supprimer le cours "${this.cours?.libelle}" ?`)) {
      this.coursService.delete(this.cours!.id).subscribe({
        next: () => {
  ...
  this.router.navigate(['/cours', this.cours?.id, 'enseignants']);
 }
 viewEnseignant(id : number): void {
  this.router.navigate(['/enseignants', id, 'enseignants']);
